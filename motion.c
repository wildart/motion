...
#elif HAVE_OMX
                (cnt->omx_output || (cnt->conf.useextpipe && cnt->extpipe))) {
#else
...
#elif HAVE_OMX
                if (cnt->omx_output || (cnt->conf.useextpipe && cnt->extpipe)) {
#else
...
#ifdef HAVE_OMX
    /*
     * OMX initialization is only performed if OMX support was found
     * and not disabled during the configure phase.
     */
    omx_init();
#endif /* HAVE_OMX */
